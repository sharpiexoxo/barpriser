      ...
      <div className="border-b border-surface-3 px-10 py-7 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-ink-3 mt-1">
            {city ? `Prisstatistik for barer i ${cityLabel}` : "Vælg en by for at se statistik"}
          </p>
        </div>
        {city && (
        ...
        {city && !loading && stats && stats.total_entries > 0 && (
          <div className="space-y-10">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <StatCard label="Antal priser" value={String(stats.total_entries)} />
              <StatCard label="Registrerede steder" value={String(stats.total_venues)} />
              <StatCard label="Gennemsnitspris"
                value={stats.overall_avg ? `${stats.overall_avg} <span class='text-base font-sans text-ink-3 font-normal'>kr</span>` : "—"} />
              <StatCard label="Prisinterval"
                value={stats.overall_min && stats.overall_max
                  ? `${stats.overall_min}–${stats.overall_max} <span class='text-base font-sans text-ink-3 font-normal'>kr</span>`
                  : "—"} />
            </div>

            <div>
              <div className="section-label mb-4">Gennemsnitspris pr. sted</div>
            ...
            {stats.by_category.length > 0 && (
              <div>
                <div className="section-label mb-4">Pr. kategori</div>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  {stats.by_category.map(c => (
                    <div key={c.category} className="card">
                      ...
                      <div className="text-[11px] text-ink-3 mt-1">{c.count} priser</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div>
              <div className="section-label mb-4">Steder — detaljeret</div>
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-surface-3">
        ...
        <div className="flex gap-5 text-right">
          ...
          <div>
            ...
            <div className="text-[10px] font-mono uppercase text-ink-3 tracking-wide">priser</div>
          </div>
        </div>
      </div>
      {entries.length > 0
