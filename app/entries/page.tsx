...
const CATEGORIES = [
  "Beer (draft)", "Beer (bottle/can)", "Wine (glass)",
  "Cocktail", "Spirit (single)", "Shot", "Soft drink", "Other",
];
...
function EntryCard({ entry, onDelete }: { entry: Entry; onDelete: () => void }) {
  ...
  return (
    <>
      <div
        className="card flex gap-4 items-start group hover:border-surface-3 transition-all"
      ...
      >
        ...
        {entry.photo_path ? (
          ...
          <img
            ...
            className="w-16 h-16 object-cover rounded-xl border border-surface-3 cursor-pointer shrink-0 hover:opacity-90 transition-opacity"
          />
        ) : (
          <div className="w-16 h-16 bg-surface-2 rounded-xl border border-surface-3 shrink-0 flex items-center justify-center text-2xl">
            🍺
          </div>
        )}
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-1.5 mt-1.5">
            {entry.category && (
              <span className="badge bg-brand-light text-brand-dark">{entry.category}</span>
            )}
            {entry.notes && (
              <span className="badge bg-surface-2 text-ink-3">{entry.notes}</span>
            )}
        ...
        <div className="text-right shrink-0">
          ...
          <button
            onClick={onDelete}
            className={clsx(
              "mt-2 btn-danger p-1.5 rounded-lg transition-opacity",
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap gap-2 mb-6 items-center">
        ...
        <button
          onClick={() => setCategory("")}
          className={clsx("chip text-xs py-1", category === "" && "chip-active")}
        >All</button>
        {CATEGORIES.map((c) => (
          <button
            key={c}
            onClick={() => setCategory(c)}
            className={clsx("chip text-xs py-1", category === c && "chip-active")}
          >{c}</button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-ink-3">
          ...
          <p className="text-sm">No entries yet — go add one!</p>
        </div>
      ) : (
        <div className="flex flex-col gap-3">
          {entries.map((e) => (
            <EntryCard key={e.id} entry={e} onDelete={() => del(e.id)} />
          ))}
        </div>
      )}
    </div>
